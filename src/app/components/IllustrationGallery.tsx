 ...
 *
 * The data file is already sequenced so similar work never lands next to
 * similar work — by hue, brightness, category and series — which means the All
 * view and every single filter are both well mixed without any runtime
 * shuffling. Masonry is built by dealing items round-robin into N columns, so
 * reading order runs left-to-right across the top row. CSS `columns` would fill each
