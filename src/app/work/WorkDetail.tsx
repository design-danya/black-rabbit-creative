 ...
 * admin draft preview. `draft` shows a banner when previewing an unpublished item.
 */
export function WorkDetail({
  ...
  return (
    <div className="bg-[#060606] text-white min-h-screen">
      {draft && (
        <div className="fixed top-0 left-0 right-0 z-[130] bg-[#5b3fd6] text-white text-center text-[11px] uppercase tracking-[0.25em] py-1.5">
      ...
      <section className="px-6 md:px-16 lg:px-24 pt-28 md:pt-36 pb-12 max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-6">
          <span className="text-[11px] uppercase tracking-[0.35em] text-[#7c5fe6]">{item.category}</span>
          {item.year && (
            <span className="text-[11px] uppercase tracking-[0.25em] text-white/40">{item.year}</span>
          )}
          {item.badge && (
            <span className="text-[10px] uppercase tracking-[0.2em] bg-[#5b3fd6] text-white px-2 py-1">{item.badge}</span>
          )}
        </div>
