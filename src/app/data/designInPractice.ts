 ...
 * Graphic Design page, in one scrolling row. Order is a pre-computed visual mix: a greedy pass followed by pair swaps
 * spaces out repeats of the same client, the same tag, and images with
 * similar hue or brightness, scored around the loop so the wrap point is
 * mixed too. Re-running the mix is fine; hand-sorting it back into client
 * runs is what it is deliberately avoiding.
 *
