/**
 * Brewery-specific FAQ. Separate from the main site FAQ so the service page
 * can answer the questions a brewery owner actually types, and emit FAQPage
 * structured data from the same source the page renders.
 */
...
export const breweryFaq = [
  ...
  },
  {
    ...
    a: 'Yes, and we would rather. We will talk to them before the design is final about substrate, ink limits, and any white-ink or varnish steps, so the file we hand over is built for that press instead of adjusted after the fact.',
  },
  {
    q: 'We only need one label, not a whole brand. Is that a project?',
  ...
  },
  {
    ...
    a: 'Yes. We are based in Portsmouth, NH and work with breweries across New England, and remotely beyond it. Most of the work happens in files and video calls either way.',
  },
  {
    q: 'What if we already have a logo we like?',
    a: 'Then we keep it. The Elm City Brewing project was explicitly a refine-and-strengthen rather than a start-over: the identity had grown inconsistent and hard for staff to use, so the work was restraint rather than replacement. Throwing out brand equity a brewery has already built is usually the wrong move.',
  },
]
